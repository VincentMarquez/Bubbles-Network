...
class APEPBubble(UniversalBubble):
    ...
    def __init__(self, object_id: str, context: SystemContext, **kwargs):
        ...
        # APEP Configuration
        self.mode = APEPMode(kwargs.get("mode", "semi_automated"))
    ...
    async def _handle_llm_query(self, event: Event):
        ...
        if self.cache_enabled and prompt_hash in self.prompt_cache:
            ...
            logger.info(f"{self.object_id}: Using cached refinement for {origin}")
        else:
            # Apply APEP refinement
            start_time = time.time()
            refined_prompt = await self._refine_prompt(prompt, origin, metadata)
            refinement_time = time.time() - start_time
            
            # Cache if successful
            if self.cache_enabled and refined_prompt != prompt:
